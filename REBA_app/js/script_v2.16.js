...
// 5. Setting up event listeners for both mouse and touch
function setupEventListeners() {
    ...
    window.addEventListener('mouseup', completeLine); // Use window to catch mouseup outside canvas
    ...
    window.addEventListener('touchend', function(e) {
        // For touch end, we use window to catch events outside canvas
        if (drawingStep === 1) {
            completeLine(e);
        }
    });
...
// 4. Ensure main function runs correctly and handles both desktop and mobile
function main() {
...
// Replace your setupEventListeners function with this version
function setupEventListeners() {
    console.log('Setting up event listeners');
    ...
    // IMPORTANT: Remove any previous event listeners
    // (This helps prevent duplicate event handlers)
    canvas.removeEventListener('mousedown', startDrawing);
    canvas.removeEventListener('mousemove', drawPreview);
    window.removeEventListener('mouseup', completeLine);
    canvas.removeEventListener('touchstart', startDrawing);
    canvas.removeEventListener('touchmove', drawPreview);
    window.removeEventListener('touchend', completeLine);
