...
function clearCanvas() {
	...
	// Disable undo button when canvas is completely cleared
	const undoButton = document.getElementById('undo-last');
	if (undoButton) {
...
function initializeCanvas() {
    ...
    // Set initial canvas size BEFORE calling other functions
    canvas.width = 500;
    canvas.height = 400;
...
function resizeCanvas() {
    ...
    if (uploadedImage) {
        ...
        // Set size based on container width while preserving aspect ratio
        let canvasWidth = containerWidth;  // DECLARE canvasWidth here
        let canvasHeight = Math.round(containerWidth * imageAspectRatio);
...
function finishLine(coords) {
    ...
    // ENABLE UNDO BUTTON AFTER ANY LINE IS DRAWN
    const undoButton = document.getElementById('undo-last');
    if (undoButton) {
        undoButton.disabled = false;
    }
