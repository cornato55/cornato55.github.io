...
function calculateAngle(point1, point2, toolType) {
	...
	// For lower leg angle (reference is upper leg)
	if (toolType === 'lower-leg' && lines['upper-leg']) {
		...
		// Calculate the interior angle directly (0-180°)
		const interiorAngle = Math.acos(cosTheta) * (180 / Math.PI);
		
		console.log(`Knee angle (interior): ${interiorAngle.toFixed(1)}°`);
...
function setupTouchEventListeners() {
    // Directly handle touch events instead of converting to mouse events
...
function stopDrawing(e) {
	...
	const returnToTool = localStorage.getItem('returnToTool');
		if (returnToTool) {
			localStorage.removeItem('returnToTool');
			selectTool(returnToTool);
		} else {
			// Normal workflow - advance to next tool
			showAdjustmentPopup(toolType);
		}
...
function updateInstructions(toolId) {
  ...
  // Improve the clearCanvas function to properly reset everything
function clearCanvas() {
  ...
  isDrawing = false;
  drawingStep = 0;
  currentTool = null; // Reset current tool to null
  ...
  // If an image is already uploaded, prompt for subject direction
  if (uploadedImage) {
    promptForSubjectDirection();
  }
  
  console.log("Canvas and UI completely reset");
}
    ...
    try {
		...
		toolButtons.forEach(btn => {
			btn.addEventListener('click', function() {
				...
				// If this line already exists, erase it to allow redrawing
				if (lines[toolType]) {
					// Store current tool to return to after redraw
					...
					// Update dependent angles if needed
					updateDependentAngles(toolType);
					
					// Redraw canvas
					redrawCanvas();
					
					// Select this tool for redrawing
					selectTool(toolId);
					
					// Set a flag to know we need to return to the previous tool
					localStorage.setItem('returnToTool', currentToolId);
				} else {
...
// Fix the displayREBAResults function to prevent duplication
function displayREBAResults(result) {
    ...
    // IMPORTANT: Remove any existing direction info and buttons to prevent duplication
    const existingDirectionInfos = document.querySelectorAll('.subject-direction-info');
    existingDirectionInfos.forEach(el => el.remove());
    
    const existingButtonContainers = document.querySelectorAll('.download-buttons-container');
    existingButtonContainers.forEach(el => el.remove());
    ...
    directionInfo.className = 'subject-direction-info'; // Add class for easy removal
    ...
    // Button container with a class for easy removal
